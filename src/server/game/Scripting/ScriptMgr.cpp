...
void ScriptMgr::OnPlayerEnterMap(Map* map, Player* player)
{
    ...
    FOREACH_SCRIPT(AllMapScript)->OnPlayerEnterAll(map, player); //autobalance
...
void ScriptMgr::OnPlayerLeaveMap(Map* map, Player* player)
{
    ...
    FOREACH_SCRIPT(AllMapScript)->OnPlayerLeaveAll(map, player); //autobalance
...
void ScriptMgr::OnMapUpdate(Map* map, uint32 diff)
{
    ...
    FOREACH_SCRIPT(AllMapScript)->OnAllUpdate(map, diff); //autobalance
...
// Unit
void ScriptMgr::ModifyHealRecieved(Unit* healer, Unit* target, uint32 &heal)// ===================> autobalance
{
    FOREACH_SCRIPT(UnitScript)->ModifyHealRecieved(healer, target, heal);// ===================> autobalance
}
...
AllMapScript::AllMapScript(const char *name) // autobalance
    : ScriptObject(name)
{
...
{
    FOREACH_SCRIPT(AllCreatureScript)->OnAllCreatureUpdate(creature, diff); // END AUTOBALANCE add
}
...
template class TC_GAME_API ScriptRegistry<AllMapScript>; //autobalance
template class TC_GAME_API ScriptRegistry<AllCreatureScript>; //autobalance
