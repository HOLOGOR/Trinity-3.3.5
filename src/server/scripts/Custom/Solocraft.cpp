...
public:
    ...
    void OnLogin(Player* Player, bool /*firstLogin*/) override
    {
        ...
        if (SoloCraftEnable)
        {
            ...
            {
                TC_LOG_INFO("server.worldserver", "execute OnLogin 314");
    ...
    void OnLogout(Player* player) override
    {
        ...
        TC_LOG_INFO("server.worldserver", "execute OnLogout 322");

        char buffer1[256];
        std::sprintf(buffer1, "DELETE FROM custom_solocraft_character_stats WHERE GUID = %u", player->GetGUID());
        std::string req1 = std::string(buffer1);
        CharacterDatabase.PQuery(req1);
...
private:
    ...
    void ApplyBuffs(Player* player, Map* map, float difficulty, int dunLevel, int numInGroup, int classBalance)
    {
        ...
        if (difficulty != 0)
        {
            ...
            if (player->GetLevel() <= dunLevel + SolocraftLevelDiff) //If a player is too high level for dungeon don't buff but if in a group will count towards the group offset balancing.
            {
                ...
                TC_LOG_INFO("server.worldserver", "execute ApplyBuffs 460"); /////////////////////

                char buffer2[256];
                std::sprintf(buffer2, "SELECT `GUID`, `Difficulty`, `GroupSize`, `SpellPower`, `Stats` FROM `custom_solocraft_character_stats` WHERE GUID = %u", player->GetGUID());
                std::string req2 = std::string(buffer2);
                QueryResult result = CharacterDatabase.PQuery(req2);
                ...
                for (int32 i = STAT_STRENGTH; i < MAX_STATS; ++i) //STATS defined/enum in SharedDefines.h
                {
                    if (result)
                    {
                        TC_LOG_INFO("server.worldserver", "execute ApplyBuffs 470"); /////////////////////
                ...
                if (player->GetPowerType() == POWER_MANA)
                {
                    ...
                    if (result)
                    {
                        ...
                        TC_LOG_INFO("server.worldserver", "execute ApplyBuffs 488"); /////////////////////
                ...
                TC_LOG_INFO("server.worldserver", "execute ApplyBuffs 514"); /////////////////


                char buffer3[256];
                std::sprintf(buffer3, "REPLACE INTO custom_solocraft_character_stats (GUID, Difficulty, GroupSize, SpellPower, Stats) VALUES (%i, %f, %d, %d, %f)", player->GetGUID(), difficulty, numInGroup, SpellPowerBonus, SoloCraftStatsMult);
                std::string req3 = std::string(buffer3);
                CharacterDatabase.PExecute(req3);


                //CharacterDatabase.PExecute("REPLACE INTO custom_solocraft_character_stats (GUID, Difficulty, GroupSize, SpellPower, Stats) VALUES (%i, %f, %i, %i, %f)", player->GetGUID(), difficulty, numInGroup, SpellPowerBonus, SoloCraftStatsMult);
            }
            else
            {
        ...
        if (group)
        {
            ...
            for (Group::member_citerator itr = groupMembers.begin(); itr != groupMembers.end(); ++itr)
            {
                ...
                if (itr->guid != player->GetGUID())
                {
                    ...
                    TC_LOG_INFO("server.worldserver", "execute GetGroupDifficulty 541"); ///////////////////


                    char buffer4[256];
                    std::sprintf(buffer4, "SELECT `GUID`, `Difficulty`, `GroupSize` FROM `custom_solocraft_character_stats` WHERE GUID = %u", itr->guid);
                    std::string req4 = std::string(buffer4);
                    QueryResult result = CharacterDatabase.PQuery(req4);
                    ...
                    if (result)
                    {
                        TC_LOG_INFO("server.worldserver", "execute GetGroupDifficulty 549"); /////////////////////
    ...
    void ClearBuffs(Player* player, Map* map)
    {
        ...
        TC_LOG_INFO("server.worldserver", "execute ClearBuffs 566"); ////////////////////player->GetGUID()
        if (player->GetGUID() != 0)
        {
            int test = player->GetGUID();
            TC_LOG_INFO("server.worldserver","test 570 %u", test);
            printf("????? ????? 571: %u \n", test);
        };

        char buffer5[256];
        std::sprintf(buffer5, "SELECT `GUID`, `Difficulty`, `GroupSize`, `SpellPower`, `Stats` FROM `custom_solocraft_character_stats` WHERE GUID = %u", player->GetGUID());
        std::string req5 = std::string(buffer5);
        QueryResult result = CharacterDatabase.PQuery(req5);

        //QueryResult result = CharacterDatabase.PQuery("SELECT `GUID`, `Difficulty`, `GroupSize`, `SpellPower`, `Stats` FROM `custom_solocraft_character_stats` WHERE GUID = %u", player->GetGUID());
        if (result)
        {
            TC_LOG_INFO("server.worldserver", "execute ClearBuffs 570"); ////////////////////
            ...
            TC_LOG_INFO("server.worldserver", "execute ClearBuffs 596"); ///////////////////

            char buffer6[256];
            std::sprintf(buffer6, "DELETE FROM custom_solocraft_character_stats WHERE GUID = %u", player->GetGUID());
            std::string req6 = std::string(buffer6);
            CharacterDatabase.PExecute(req6);
